...
export async function classifyRisk(params: {
  ...
  try {
    ...
    if (!result.success) {
      ...
      // SAFETY: On invalid format, assume worst case — NOT permissive L2.
      // Same as total API failure: L3_PLUS prevents reframing and includes professional referral.
      return {
        risk_level: 'L3_PLUS',
  ...
  try {
    ...
    if (!result.success) {
      ...
      const risk: RiskAssessment = riskResult.success
        ? riskResult.data
        : { risk_level: 'L3_PLUS', topic_category: FALLBACK_TOPIC_CATEGORY, action_required: 'Fallback — invalid combined response', reasoning: 'Automatic RESTRICTIVE fallback due to parsing error — classified as L3_PLUS for safety' };
